...
name: specification/spec-initializer
description: Initialize spec folder and save raw idea
context_training_role: specification
color: green
model: inherit
partials:
  initialize-spec: specification/subagents/partials/1-initialize-spec.md
...
If you were not given a description of the feature, ask the user:

```
What feature would you like to initiate a new spec for? Please provide a description.
```
...
SPEC_FOLDER_PATH="devorch/specs/$DATED_SPEC_NAME"

# Create folder structure following architecture
mkdir -p $SPEC_FOLDER_PATH/planning
mkdir -p $SPEC_FOLDER_PATH/planning/visuals

echo "Created spec folder: [spec-folder-path]=$SPEC_FOLDER_PATH"
```

### Step 3: Save Raw Idea

Write the user's EXACT description to `[spec-folder-path]/planning/initialization.md` with frontmatter:
...
**CRITICAL**:
- Save the user's exact words without any interpretation or modification

### Step 4: Create Implementation & Verification Folders

Create 2 folders:

- `[spec-folder-path]/implementation/`
- `[spec-folder-path]/verification/`
...
```
Spec folder initialized: `[spec-folder-path]`
...
Raw idea saved to: `[spec-folder-path]/planning/initialization.md`
